...
import { UploadedCandidate, Interview, College, Drive } from "@server/lib/db";
...
function getCandidateRoundResults(
  ...
  const getRoundStatus = (
    ...
    if (submittedPanels.length > 0) {
      const hasRejected = submittedPanels.some(
...
export const CandidatesTab = ({
  ...
  const [singleCandidateCollegeDrive, setSingleCandidateCollegeDrive] =
    useState("");
  const [isAddingSingleCandidate, setIsAddingSingleCandidate] = useState(false);
  const [singleCandidateError, setSingleCandidateError] = useState<
    string | null
  >(null);
  ...
  const [editCandidateCollegeDrive, setEditCandidateCollegeDrive] =
    useState("");
  const [editCandidateDate, setEditCandidateDate] = useState("");
  const [mappingCandidateId, setMappingCandidateId] = useState<string | null>(
    null,
  );
  const [selectingCandidateId, setSelectingCandidateId] = useState<
    string | null
  >(null);
  const [unmappingCandidateId, setUnmappingCandidateId] = useState<
    string | null
  >(null);
  ...
  const [candidateStatusFilter, setCandidateStatusFilter] = useState<
    "all" | "WAITING" | "MAPPED"
  >("all");
  const [candidateCollegeFilter, setCandidateCollegeFilter] =
    useState<string>("all");
  const [candidateDateFilter, setCandidateDateFilter] = useState<string>("all");
  const [scopeToActiveDrive, setScopeToActiveDrive] =
    useState<boolean>(!!activeDrive);
  const [isLoadingCandidates, setIsLoadingCandidates] = useState(false);
  const [selectedFeedbackCandidate, setSelectedFeedbackCandidate] =
    useState<UploadedCandidate | null>(null);
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      uploadDefaultCollege === "_none_placeholder" ||
      uploadDefaultCollege.trim() === ""
    ) {
      setUploadError(
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < json.length; i++) {
          ...
          if (nameKey && emailKey) {
            ...
            const preferredDate =
              ...
              (uploadDefaultDate ? uploadDefaultDate : undefined);

            const driveCollegeKey = keys.find((k) => {
              const val = normalizeHeader(k);
              return (
                val === "college name of drive" ||
                val === "drive college" ||
                val === "college of drive"
              );
            });
            ...
            const candidateCollegeKey = keys.find((k) => {
              const val = normalizeHeader(k);
              return (
                val === "college name of candidate" ||
                val === "candidate college" ||
                val === "college" ||
                val === "institution" ||
                val === "university" ||
                val === "college name"
        ...
        console.error(err);
        setUploadError(
          err.message || "An error occurred during file parsing or upload.",
        );
      } finally {
        setIsUploadingCandidates(false);
        e.target.value = "";
  ...
  const handleAddSingleCandidate = async (e: React.FormEvent) => {
    ...
    if (!singleCandidateCollegeDrive.trim()) {
    ...
    try {
      ...
      toast.success(
        `Candidate ${singleCandidateName} successfully added to the queue!`,
      );
    } catch (err: any) {
      console.error(err);
      setSingleCandidateError(
        err.message || "An error occurred adding candidate.",
      );
    } finally {
  ...
  const handleSaveCandidateEdit = async (id: string) => {
    if (!editCandidateName.trim()) {
      toast.error("Name is required.");
      return;
    }
    if (!editCandidateEmail.trim()) {
      toast.error("Email is required.");
      return;
    }
    if (!editCandidateCollege.trim()) {
      toast.error("College Name of Candidate is required.");
      return;
    }
    if (!editCandidateCollegeDrive.trim()) {
      toast.error("College Name of Drive is required.");
      return;
    }
    if (!editCandidateDate.trim()) {
  ...
  const handleMapCandidateToSlot = async (
    ...
    try {
      ...
      const data = await res.json();
      setInterviews(
        interviews.map((i) => (i.id === interviewId ? data.interview : i)),
      );
      await fetchCandidates();
      toast.success(
        `Candidate "${candidate.name}" successfully mapped to the interview!`,
      );
    } catch (err: any) {
      console.error(err);
  ...
  const handleUnmapCandidate = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/candidates/${id}/unmap`, {
        method: "POST",
      });
      const result = await res.json();
      if (!res.ok)
        throw new Error(result.error || "Failed to unmap candidate.");
      setCandidates(result.candidates);
      setInterviews(result.interviews);
  ...
  const handleResumeUpload = async (candidateId: string, file: File) => {
    ...
    try {
      ...
      setCandidates(result.candidates);
      toast.success(
        "Resume attached. Panelists on this candidate's interview can now use the AI Copilot.",
      );
    } catch (err: any) {
    ...
    try {
      const dataToExport = filteredCandidates.map((candidate) => {
        ...
        return {
          ...
          "Queue Status":
            candidate.status === "MAPPED" || !!mappedIntv
              ? "Mapped"
              : "Waiting",
          "L1 Result": l1Result,
          "L2 Result": l2Result,
  ...
  const filteredCandidates = candidates.filter((c) => {
    ...
    const matchesCollege =
      ...
      (c.collegeDrive &&
        c.collegeDrive.toLowerCase() ===
          candidateCollegeFilter.toLowerCase()) ||
      (c.college &&
        c.college.toLowerCase() === candidateCollegeFilter.toLowerCase());
    const matchesDate =
    ...
    const matchesActiveDrive =
      ...
      (((c.collegeDrive &&
        c.collegeDrive.toLowerCase() ===
          activeDrive.collegeName.toLowerCase()) ||
        (c.college &&
      ...
      >
        <div>
          ...
          <p className="text-muted text-sm" style={{ margin: "4px 0 0 0" }}>
            Manage candidate uploads, authorization, and interview mapping.
          </p>
        </div>
      </div>
      ...
      <div className="candidates-layout">
        ...
        <div className="glass-card" style={{ padding: "1.25rem" }}>
          ...
          <p
            ...
            style={{ marginBottom: "1.25rem", marginTop: "4px" }}
          >
          ...
          >
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label
                ...
                style={{ fontSize: "0.8rem", fontWeight: 600 }}
              >
                College Name of Drive (Fallback Default)
              </label>
              <Select
                value={uploadDefaultCollege}
                onValueChange={(val) => setUploadDefaultCollege(val || "")}
              >
                <SelectTrigger
          ...
          <form
            onSubmit={handleAddSingleCandidate}
            style={{ display: "flex", flexDirection: "column", gap: "1rem" }}
          >
            <h4
              style={{
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              <Select
                value={singleCandidateCollegeDrive}
                onValueChange={(val) =>
                  setSingleCandidateCollegeDrive(val || "")
                }
              >
                <SelectTrigger
                  className="w-full text-left"
            ...
            <button
              ...
              disabled={
                ...
                !singleCandidateName.trim() ||
                !singleCandidateEmail.trim()
              }
        ...
        <div className="glass-card" style={{ padding: "1.25rem" }}>
          ...
          <div className="filter-toolbar">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(val) => setCandidateStatusFilter(val as any)}
              >
                <SelectTrigger
            ...
            <div>
              ...
              <Select
                value={candidateCollegeFilter}
                onValueChange={(val) => setCandidateCollegeFilter(val || "all")}
              >
                <SelectTrigger
                  className="text-left w-full"
                  style={{
            ...
            <div>
              ...
              >
                <input
                  type="date"
                  value={
                    candidateDateFilter === "all" ? "" : candidateDateFilter
                  }
                  onChange={(e) =>
                    setCandidateDateFilter(e.target.value || "all")
                  }
                  style={{
                    width: "100%",
            ...
            <div className="checkbox-field">
              <label
                style={{
                  ...
                  gap: "0.5rem",
                  fontSize: "0.75rem",
                  cursor: activeDrive ? "pointer" : "not-allowed",
                  color: activeDrive ? "inherit" : "var(--text-muted)",
                  userSelect: "none",
                }}
              >
                <input
          ...
          <div className="queue-summary">
            <span>
              Total Candidates: <strong>{filteredCandidates.length}</strong>
            </span>
            <span>
              Mapped:{" "}
              <strong>
                {filteredCandidates.filter((c) => c.status === "MAPPED").length}
              </strong>
            </span>
            <span>
              Waiting:{" "}
              <strong>
                {
                  filteredCandidates.filter((c) => c.status === "WAITING")
                    .length
                }
              </strong>
            </span>
            {activeDrive && (
              <span>
                Active Drive: <strong>{activeDrive.collegeName}</strong>
              </span>
            )}
          </div>
              ...
              <table className="queue-table">
                ...
                <tbody>
                  {filteredCandidates.map((candidate) => {
                    let mappedIntv = candidate.mappedInterviewId
                      ? interviews.find(
                          (i) => i.id === candidate.mappedInterviewId,
                        )
                      : null;
                          ...
                          <>
                            ...
                            <td style={{ padding: "0.5rem 1rem" }}>
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditCandidateCollege(e.target.value)
                                  }
                                  required
                                ...
                                <Select
                                  ...
                                  onValueChange={(val) =>
                                    setEditCandidateCollegeDrive(val || "")
                                  }
                                >
                                  <SelectTrigger
                                    className="text-left"
                            ...
                            <td style={{ padding: "0.5rem 1rem" }}>
                              <input
                                ...
                                onChange={(e) =>
                                  setEditCandidateDate(e.target.value)
                                }
                                min={todayStr}
                                required
                              />
                            ...
                            >
                              ...
                              >
                                <button
                                  onClick={() =>
                                    handleSaveCandidateEdit(candidate.id)
                                  }
                                ...
                                const getStatusSpan = (
                                  ...
                                  if (result === "Pending Feedback")
                                    return (
                                      <span>
                                        <strong>{name}:</strong>{" "}
                                        <span className="result-status-warning">
                                          Pending
                                        </span>
                                      </span>
                                    );
                                  ...
                                  >
                                    <Select
                                      onValueChange={async (val: any) => {
                                        if (
                                          val &&
                                          val !== "_none_placeholder"
                                        ) {
                                          await handleMapCandidateToSlot(
                                            candidate,
                                            val,
                                          );
                                  ...
                                  <>
                                    ...
                                    {isMapped && (
                                      <ConfirmDialog
                                        ...
                                        onConfirm={() =>
                                          handleUnmapCandidate(candidate.id)
                                        }
                                      />
                                    )}
                                    <button
                                      onClick={() => {
                                        ...
                                        setEditCandidateCollege(
                                          candidate.college || "",
                                        );
                                        setEditCandidateCollegeDrive(
                                          candidate.collegeDrive || "",
                                        );
                                        setEditCandidateDate(
                                          candidate.preferredDate || "",
                                        );
                                      }}
                                      className="row-action-button"
                                    ...
                                    {(candidate as any).outcomeStatus ===
                                      "PASSED_L2" && (
                                      <ConfirmDialog
                                        trigger={
                                          <button
                                        ...
                                        destructive={false}
                                        onConfirm={() =>
                                          handleMarkAsSelected(candidate.id)
                                        }
                                      />
                                    )}
                                    <ConfirmDialog
                  ...
                  {filteredCandidates.length === 0 && (
                    <tr>
                      ...
                      >
                        No candidates registered in the queue. Use the actions
                        panel on the left to add candidates.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          >
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                >
                  {selectedFeedbackCandidate.email} ·{" "}
                  {selectedFeedbackCandidate.college}
                </p>
              </div>
              <button
            ...
            >
              ...
              {["L1", "L2"].map((round) => {
                const roundInterviews = interviews.filter(
                  (i) =>
                    i.candidateEmail.toLowerCase() ===
                      selectedFeedbackCandidate.email.toLowerCase() &&
                          ...
                          >
                            ...
                            {submittedPanels.length === 0 &&
                              pendingPanels.length === 0 && (
                                ...
                                >
                                  No panel members nominated for this interview
                                  slot.
                                </p>
                              )}

                            {submittedPanels.map((panel) => {
                              const parsed = parseFeedbackSafely(
                                panel.feedback,
                              );
                            ...
                            {pendingPanels.map((panel) => (
                              ...
                              >
                                <span>
                                  Panelist: <strong>{panel.name}</strong> (
                                  {panel.email})
                                </span>
                                <span
                                  className="badge badge-pending"
