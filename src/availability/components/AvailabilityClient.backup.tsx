...
export const AvailabilityClient = ({
  ...
  const handleBookSelectedSlots = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(
        (err as Error).message || "An error occurred while booking selected slots.",
      );
    } finally {
      setIsBooking(false);
    }
  ...
  const handleRejectRequest = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(
        (err as Error).message || "An error occurred while declining the request.",
      );
    } finally {
  ...
  const handleSubmitSlots = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg((err as Error).message || "Error occurred while saving availability.");
    } finally {
        ...
        >
          {panel.availabilities.length === 0 ? (
            ...
            panel.availabilities.map((slot) => {
              ...
              const isPast = start.getTime() < currentTime;
