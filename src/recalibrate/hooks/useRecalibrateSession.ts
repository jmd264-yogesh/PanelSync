...
import type { RoleGrade } from '@server/services/ai/spec-catalog';
import { ROLE_GRADES, STYLES } from '@server/services/ai/spec-catalog';
import {
...
} from '@server/services/ai/org-rubric';
import { buildCandidateSheetHtml, buildPanelistReportHtml, printHtmlDocument } from '@server/util/pdf/recalibrate-print';

export type TRubricBand = {
  band: string;
  description: string;
  exampleSignals: string[];
}

export type TRecalibrateQuestion = {
  id: string;
  category: string;
  question: string;
  intent: string;
...
export type TQuestionSet = {
  questions: TRecalibrateQuestion[];
  totalMarks: number;
...
export const useRecalibrateSession = ({
  ...
  const [session, setSession] = useState<TRecalibrateSession | null>(null);
  const [activeRun, setActiveRun] = useState<TAiRun | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        let loadedSession: TRecalibrateSession | null = null;
  ...
  const technicalDims: TRubricDim[] = useMemo(() => TECHNICAL_CATEGORIES_BY_TIER[orgTier].map((id) => ({
    label: TECHNICAL_CATEGORY_LABEL[id],
    bands: TECHNICAL_RUBRIC[orgTier][id]!,
  })), [orgTier]);
  const behaviouralDims: TRubricDim[] = useMemo(() => BEHAVIOURAL_CATEGORIES.map((id) => ({
    label: BEHAVIOURAL_CATEGORY_LABEL[id],
    bands: BEHAVIOURAL_RUBRIC[id],
  })), []);
  const allDims: TRubricDim[] = useMemo(() => [...technicalDims, ...behaviouralDims], [technicalDims, behaviouralDims]);
