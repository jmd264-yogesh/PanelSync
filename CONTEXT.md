...
- `deriveFocusAreas(roleGrade, selectedTechStacks?)` — the tier's technical category labels (narrowed to `selectedTechStacks` if given — the panelist's per-candidate tech-stack picks in `Spec.techStacks`; falls back to the full tier list) + the 4 behavioural labels; this is the AI prompt's allowed "category" list **and** `verifyQuestionSet`'s `focusAreas`.
- `rubricDimensionsWithBands(roleGrade, selectedTechStacks?)` — the same combined (optionally narrowed) list, each with its `[score1, score2, score3, score4]` description array, for the Overall Scoring Rubric grid.
